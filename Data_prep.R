...
## For the other 6 instances, the raw data still disagree - to avoid making judgement calls, we're going to drop these 8
issues_inf <- infs_dat %>% 
...
## going in and making the subplot count for plot 1, subplot 1 in 2014 4 instead of 3, based on the raw data sheet
...
## df for linear regression conversion from seed mass to seed counts
...
## select just the values and dataframes that will be used to parameterize the JAGS model 
keep = c("N.trt",
         "N.years",
         "N.plots",
         "N.obs",
...
## drop all of the other dfs
rm(list=setdiff(ls(), keep))

## save workspace to load .RDATA and run JAGS model

save.image("Data_for_JAGS_Model.RData")
